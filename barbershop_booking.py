...
class BarberShopBooking:
    ...
    def show_time_slot_page(self):
        ...
        tk.Label(time_slot_window, text="Available Time Slots").pack()
        
        for day in days:
            frame = tk.Frame(time_slot_window)
            frame.pack(anchor="w")
            ...
            for hour in hours:
                ...
                if is_booked:
                    tk.Button(frame, text=f"{hour} - Unavailable", state=tk.DISABLED).pack(side="left", padx=2)
                else:
                    tk.Button(frame, text=hour, command=lambda s=combined_slot: self.select_time_slot(s, time_slot_window), activebackground="yellow").pack(side="left", padx=2)
